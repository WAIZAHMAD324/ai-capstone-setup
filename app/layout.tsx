    ...
    <html lang="en">
      ...
      >
              ...
              <ul className="absolute right-0 mt-2 w-48 rounded-md border border-[var(--border)] bg-[var(--card)] p-2 shadow-lg">
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="block rounded-md px-3 py-2 text-sm font-medium hover:bg-[var(--secondary)]"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <footer className="border-t border-[var(--border)] py-6">
          <div className="mx-auto max-w-7xl px-4 text-center text-sm text-[var(--muted)] sm:px-6 lg:px-8">
